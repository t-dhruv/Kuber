...
- CI runs lint, server unit tests, typecheck, and build, but explicitly skips e2e because PostgreSQL and seeded data are not configured in CI (`.github/workflows/ci.yml:33`, `.github/workflows/ci.yml:45`, `.github/workflows/ci.yml:53`).
...
3. Tests must be deterministic, isolated by household/user, and runnable in CI from a clean checkout.
4. Bugs found after merge get regression tests before the fix is accepted.
5. CI should fail fast on cheap checks and run deeper checks before merge or release.
...
| Unit tests | yes | yes | all workspace unit tests pass |
...
| Full e2e regression | no, unless touched area is critical | yes | all browser journeys pass in CI |
...
5. Add household isolation tests for all financial routes.
6. Add migration verification to CI for Prisma changes.
7. Add release workflow gate for full e2e regression.
8. Add QA dashboard section to PR summary with gate status and artifact links.
