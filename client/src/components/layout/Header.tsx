...
export function Header({ onToggleSidebar, notificationCount = 0 }: HeaderProps) {
  ...
  return (
    <header className="h-14 flex items-center justify-between gap-4 px-4 border-b border-[var(--color-border)] bg-[var(--color-surface)] flex-shrink-0">
      ...
      <div className="flex items-center gap-1">
        ...
        {/* User avatar */}
        <div className="ml-1">
          <Avatar name={user ? `${user.firstName} ${user.lastName}` : 'User'} size="sm" className="cursor-pointer" />
        </div>
      </div>
    </header>
  );
