...
import { SlidersHorizontal, BookmarkPlus, ChevronDown } from 'lucide-react';
            ...
            {DATE_PRESETS.map((preset) => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  backgroundColor: value === preset.value ? 'var(--color-accent-light)' : 'transparent',
                  color: value === preset.value ? 'var(--color-accent)' : 'var(--color-text)',
                  fontWeight: value === preset.value ? 600 : 400,
                }}
                onMouseEnter={(e) => {
                  if (value !== preset.value) e.currentTarget.style.backgroundColor = 'var(--color-surface-hover)';
                }}
                onMouseLeave={(e) => {
      ...
      {TABS.map((t) => (
        <button
          ...
          style={{
            ...
            color: tab === t.value ? 'var(--color-accent)' : 'var(--color-text-secondary)',
            border: 'none',
...
function CashFlowTab({
  ...
  const { data, isLoading } = useQuery<CashFlowReport>({
    ...
    queryFn: () =>
      api.get(`/reports/cashflow?startDate=${startDate}&endDate=${endDate}`).then((r) => r.data),
  });
            ...
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height={240}>
                <ComposedChart data={chartData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                  ...
                  />
                  <Bar dataKey="income" name="income" fill="var(--color-success)" radius={[3, 3, 0, 0]} maxBarSize={28} opacity={0.85} />
                  <Bar dataKey="expenses" name="expenses" fill="var(--color-danger)" radius={[3, 3, 0, 0]} maxBarSize={28} opacity={0.85} />
                  <Line
                    type="monotone"
                    dataKey="net"
                    name="net"
...
function CategoryTab({ mode, startDate, endDate }: CategoryTabProps) {
  ...
  const { data: reportData, isLoading: reportLoading } = useQuery<SpendingReport | IncomeReport>({
    queryKey: [`reports-${mode}`, startDate, endDate, groupBy],
    queryFn: () =>
      api
        .get(`/reports/${mode}?startDate=${startDate}&endDate=${endDate}&groupBy=${groupBy}`)
        .then((r) => r.data),
    enabled: true,
  });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
          {/* Group by */}
          <GroupByDropdown value={groupBy} onChange={setGroupBy} options={GROUP_BY_OPTIONS} />
            ...
            {CHART_TYPES.map((ct) => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  backgroundColor: chartType === ct.value ? 'var(--color-surface)' : 'transparent',
                  color: chartType === ct.value ? 'var(--color-text)' : 'var(--color-text-secondary)',
                  boxShadow: chartType === ct.value ? 'var(--shadow-sm)' : 'none',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '55% 45%', gap: '1rem', alignItems: 'start' }}>
        {/* Left: chart */}
        <Card padding="lg">
          {reportLoading ? (
              ...
              <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                {displayedCategories.map((cat, i) => (
                  <div key={cat.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    <span style={{ fontSize: '0.8125rem', color: 'var(--color-text)', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        ...
        <Card padding="lg">
          {reportLoading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {[1, 2, 3, 4, 5].map((i) => <Skeleton key={i} height={24} width="100%" />)}
            </div>
          ) : (
            <>
              {/* Summary stats */}
              ...
              <div style={{ marginTop: '1rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                  ...
                  {categories.length === 0 && (
                    <span style={{ fontSize: '0.875rem', color: 'var(--color-text-muted)' }}>No data.</span>
                  )}
                </div>
              </div>
            </>
      ...
      <Card padding="lg">
        ...
        {txLoading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {[1, 2, 3, 4, 5].map((i) => <Skeleton key={i} height={48} width="100%" />)}
          </div>
      ...
      <ResponsiveContainer width="100%" height={240}>
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(value: number) => [fmtCurrency(value), 'Amount']}
          />
          <Bar dataKey="value" radius={[0, 3, 3, 0]} maxBarSize={20} animationDuration={600}>
            {data.map((entry, i) => (
              <Cell key={`cell-${i}`} fill={entry.color} />
            ))}
          </Bar>
            ...
            {options.map((opt) => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  backgroundColor: value === opt.value ? 'var(--color-accent-light)' : 'transparent',
                  color: value === opt.value ? 'var(--color-accent)' : 'var(--color-text)',
                  fontWeight: value === opt.value ? 600 : 400,
                }}
                onMouseEnter={(e) => {
                  if (value !== opt.value) e.currentTarget.style.backgroundColor = 'var(--color-surface-hover)';
                }}
                onMouseLeave={(e) => {
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.5rem', alignItems: 'baseline' }}>
      ...
      <span style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--color-text)', textAlign: 'right', maxWidth: '55%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {value}
      </span>
    </div>
  );
...
function TransactionRow({ txn, isLast }: { txn: ReportTransaction; isLast: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {isNegative ? '−' : '+'}{fmtCurrency(txn.amount)}
      </div>
    </div>
  );
}
...
export default function ReportsPage() {
  ...
  const { startDate, endDate } = useMemo(() => computeDateRange(datePreset), [datePreset]);
